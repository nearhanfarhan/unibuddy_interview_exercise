...
describe('MessageData', () => {
  ...
  describe('tags', () => {
    it('should add or update tags on a message', async () => {
      const conversationId = new ObjectID();
      const message = await messageData.create(
        { conversationId, tags: ['test'], text: 'Message with tags' },
        senderId,
      );

      expect(message.tags).toEqual(['test']);

      const updatedMessage = await messageData.addTags(message.id, ['test', 'added-tag']);
      expect(updatedMessage.tags).toEqual(['test', 'added-tag']);
